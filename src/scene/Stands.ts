...
const SAFETY_GAP  = 5.0;   // [m] touchline → stand front barrier
const FIRST_ROW_H = 1.0;   // [m] first row floor above pitch
const ROW_DEPTH   = 0.85;  // [m] tread depth per row (UEFA minimum 0.80 m)
const ROW_RISE    = 0.32;  // [m] riser height per row — governs the rake angle.
...
const NUM_ROWS    = 34;    // rows in lower tier.
                            // Total height above pitch = FIRST_ROW_H + NUM_ROWS × ROW_RISE
                            //   = 1.0 + 34 × 0.32 ≈ 11.9 m  (Bernabeu lower: ~12 m ✓)
                            // Stand depth = NUM_ROWS × ROW_DEPTH = 34 × 0.85 ≈ 29 m
const STAND_W     = FIELD_W + 10; // [m] stand width (5 m structural overhang each end)
